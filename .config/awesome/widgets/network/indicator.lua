...
local function worker(args)
  ...
  local function get_interfaces()
    ...
    local ipl_pattern = "^%d+:%s+([^%s]+):%s+<.*>%s.*%s" .. "state%s+([^%s]+)%s.*%slink/([^%s]+)[%s]*([%x:]*)"
    ...
    if not args.skiproutes then
      ...
      for iface, s in pairs(links) do
        if s.coverage then
          ...
          table.sort(s.coverage, function(a, b)
            return a[1] < b[1]
          end)
  ...
  local function text_grabber()
    ...
    for _, s in pairs(real_interfaces) do
      ...
      for a = 1, #s.addrs - 1 do
        msg = msg .. "├ADDR:\t" .. s.addrs[a].addr .. " (" .. s.addrs[a].type_ .. ")\n"
      end
      if args.skiproutes then
        msg = msg .. "└ADDR:\t" .. s.addrs[#s.addrs].addr .. " (" .. s.addrs[#s.addrs].type_ .. ")</span>\n"
      else
        msg = msg .. "├ADDR:\t" .. s.addrs[#s.addrs].addr .. " (" .. s.addrs[#s.addrs].type_ .. ")\n"
      end
