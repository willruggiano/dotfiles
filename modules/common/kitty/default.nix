    ...
    home = {
      ...
      dataFile = {
        "dark-mode.d/kitty".source = pkgs.writeShellScript "dark-kitty" ''
          ln -sf ${./base16-tomorrow-night-eighties.conf} "$HOME/.config/kitty/current-theme.conf" && kill -SIGUSR1 $(pgrep kitty)
        '';
        "light-mode.d/kitty".source = pkgs.writeShellScript "dark-kitty" ''
          ln -sf ${./base16-tomorrow.conf} "$HOME/.config/kitty/current-theme.conf" && kill -SIGUSR1 $(pgrep kitty)
        '';
      };
    };
  };
